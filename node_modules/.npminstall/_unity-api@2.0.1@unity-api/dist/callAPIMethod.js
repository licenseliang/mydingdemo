...
function callAPI() {
    ...
    var accumulatedFetchOptions = _extends({}, defaults.fetchOptions, fetchOptions, options);
    if (headers) accumulatedFetchOptions.headers = headers;
    if (body) accumulatedFetchOptions.body = body;

    return fetch(url, accumulatedFetchOptions).then(function (response) {
        return response[method]().catch(function () {
            if (!response.ok) throw new _error2.default(response.status, response.statusText);

            return response.body || null;
        }).then(function (result) {
            if (!response.ok) throw new _error2.default(response.status, response.statusText, result);
